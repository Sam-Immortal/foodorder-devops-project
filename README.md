...
src/ main/ java/com/restaurant/foodorder/ FoodorderApplication.java Spring Boot entry point controller/ REST endpoints and request handling model/ Entity classes for data models repository/ JPA repositories for data access resources/ Configuration files and static assets test/ Unit and integration tests pom.xml Maven project configuration Dockerfile Multi-stage Docker build Jenkinsfile CI/CD pipeline definition .gitignore Git ignore rules
...
The Jenkinsfile automates the entire build and deployment process:
    ...
    Docker Build – Builds the Docker image as foodorder-app:latest
    Deploy Local Server – Stops any existing container and starts a fresh one
